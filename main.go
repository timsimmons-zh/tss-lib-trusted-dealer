...
func main() {
	...
	preImp, _ := keygen.GeneratePreParams(1 * time.Minute)
	preSigners := make([]*keygen.LocalPreParams, 3)
	var err error
	for i := range signerParties {
		fmt.Printf("Computing local PreParams for signer %d\n", i)
		preSigners[i], err = keygen.GeneratePreParams(1 * time.Minute)
		if err != nil {
	...
	signerEndCh := make(chan result, 3)
	...
	impSave := keygen.NewLocalPartySaveData(1)
	impSave.LocalPreParams = *preImp
	impSave.LocalSecrets = keygen.LocalSecrets{
		Xi:      plaintextKey,
	...
	partyMap := make(map[string]*resharing.LocalParty)
	var importerPartyInstance *resharing.LocalParty
	var signerPartyInstances [3]*resharing.LocalParty

	// Create all parties
	discardEndCh := make(chan result, 1) // importer results can be ignored
	importerPartyInstance = resharing.NewLocalParty(
		impParams,
		impSave,
		makeOutCh(importerParty, outCh),
		makeEndCh(importerParty, discardEndCh),
	...
	for i, pid := range signerParties {
		...
		signerSave := keygen.NewLocalPartySaveData(1)
		...
		signerPartyInstances[i] = resharing.NewLocalParty(
			signerParams[i],
			signerSave,
			makeOutCh(pid, outCh),
			makeEndCh(pid, signerEndCh),
		).(*resharing.LocalParty)
...
func makeEndCh(pid *tss.PartyID, endCh chan result) chan *keygen.LocalPartySaveData {
	ch := make(chan *keygen.LocalPartySaveData, 1)
	go func() {
		for sd := range ch {
			endCh <- result{pid: pid, data: *sd}
		}
	}()
	return ch
}
